...
package com.wolvencraft.yasp.db.hooks.worldguard;
...
public class SerializableRegion {
    ...
    private SerializableRegion(String regionName, int priority) {
    ...
    public static String serialize(Map<String, Integer> regions) {
        List<SerializableRegion> values = new LinkedList<SerializableRegion>();
        Iterator<Entry<String, Integer>> it = regions.entrySet().iterator();
        while(it.hasNext()) {
            Map.Entry<String, Integer> pairs = (Map.Entry<String, Integer>) it.next();
            values.add(new SerializableRegion(pairs.getKey(), pairs.getValue()));
            it.remove();
        }
