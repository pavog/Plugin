package com.wolvencraft.yasp.EventDataHandlers;
...
public class EDHPlayer {
	public void PlayerBlockBreak(final Player player, final Integer blockID) {
		this.execute(player, new Runnable() {
			@Override
			public void run() {
				StatsPlugin.getInstance().getPlayerData().addBlockBreak(player.getUniqueId().toString(), blockID);
			}
		});
	}
	...
	public void PlayerBlockPlace(final Player player, final Integer blockID) {
		this.execute(player, new Runnable() {
			@Override
			public void run() {
				StatsPlugin.getInstance().getPlayerData().addBlockPlaced(player.getUniqueId().toString(), blockID);
			}
		});
	}
	...
	public void PlayerDroppedItem(final Player player, final Integer itemID, final Integer numberInStack) {
		this.execute(player, new Runnable() {
			@Override
			public void run() {
				StatsPlugin.getInstance().getPlayerData().addItemDropped(player.getUniqueId().toString(), itemID, numberInStack);
			}
		});
	}
		...
		this.execute(player, new Runnable() {
			...
			public void run() {
				try {
					...
					// Watch them
					StatsPlugin.getInstance().getPlayerData().addPlayerToWatch(player.getUniqueId().toString(), player.getLocation());
				}
			}
		});
	}
	...
	public void PlayerMove(final Player player, final Class<? extends Entity> vehicleType) {
		this.execute(player, new Runnable() {
			@Override
			public void run() {
				StatsPlugin.getInstance().getPlayerData().incrementStepsTaken(player.getUniqueId().toString(), player.getLocation(), vehicleType);
			}
		});
	}
	...
	public void PlayerPickedUpItem(final Player player, final Integer itemID, final Integer numberInStack) {
		this.execute(player, new Runnable() {
			@Override
			public void run() {
				StatsPlugin.getInstance().getPlayerData().addItemPickup(player.getUniqueId().toString(), itemID, numberInStack);
			}
		});
	}
	...
	public void PlayerKilledByPlayer(final Player killer, final Player victim, final DamageCause cause) {
		if (!StatsPlugin.getInstance().statExempt(victim)) return;
	...
	private void execute(final Player player, final KillTag killTag) {
		this.execute(player, new Runnable() {
			@Override
			public void run() {
				StatsPlugin.getInstance().getPlayerData().addKillTag(player.getUniqueId().toString(), killTag);
			}
		});
	}

	private void execute(Player player, Runnable runnable) {
		if (!StatsPlugin.getInstance().statExempt(player)) return;
		if (runnable != null) {
			StatsPlugin.getInstance().getExecutor().execute(runnable);
		}
	}
}
