...
public class SessionCache extends CachedData {
                ...
                for(Player player : Bukkit.getServer().getOnlinePlayers()) {
                    if(StatPerms.Statistics.has(player))
                        ((PlayerDataStore) fetch(player, true)
                                .getDataStore(com.mctrakr.db.data.DataStore.ModuleType.Player))
    ...
    public void clearCache() {
        for(OnlineSession session : getOnlineSessions()) {
            ...
            if(delay == 0) continue;
            long totalPlaytime = ((PlayerDataStore) session.getDataStore(com.mctrakr.db.data.DataStore.ModuleType.Player)).get().getTotalPlaytime();
