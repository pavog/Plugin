...
public class ItemsEventHandler {
    ...
    public static class ItemPickup implements Runnable {
        ...
        public void run() {
            ...
            ItemsDataStore items = ((ItemsDataStore) session.getDataStore(PrimaryType.Items));
            if(items != null) items.itemPickUp(location, itemStack);
        }
    }
    ...
    public static class ItemDrop implements Runnable {
        ...
        public void run() {
            OnlineSession session = SessionCache.fetch(player);
            ItemsDataStore items = ((ItemsDataStore) session.getDataStore(PrimaryType.Items));
            if(items != null) items.itemDrop(location, itemStack);
        }
    }
    ...
    public static class FoodConsume implements Runnable {
        ...
        public void run() {
            ...
            ItemsDataStore items = ((ItemsDataStore) session.getDataStore(PrimaryType.Items));
            if(items != null) items.itemUse(player.getLocation(), player.getItemInHand());
            
            session.getPlayerTotals().snacksEaten();
            
            MiscDataStore misc = ((MiscDataStore) session.getDataStore(PrimaryType.Misc));
            if(misc != null) misc.getNormalData().incrementStat(MiscInfoTable.FoodEaten);
        }
    }
    ...
    public static class ItemCraft implements Runnable {
        ...
        public void run() {
            OnlineSession session = SessionCache.fetch(player);
            ItemsDataStore items = ((ItemsDataStore) session.getDataStore(PrimaryType.Items));
            if(items != null) items.itemCraft(location, itemStack);
    ...
    public static class ItemSmelt implements Runnable {
        ...
        public void run() {
            OnlineSession session = SessionCache.fetch(player);
            ItemsDataStore items = ((ItemsDataStore) session.getDataStore(PrimaryType.Items));
            if(items != null) items.itemSmelt(location, itemStack);
        }
    }
    ...
    public static class ToolBreak implements Runnable {
        ...
        public void run() {
            OnlineSession session = SessionCache.fetch(player);
            ItemsDataStore items = ((ItemsDataStore) session.getDataStore(PrimaryType.Items));
            if(items != null) items.itemBreak(location, itemStack);
    ...
    public static class ItemEnchant implements Runnable {
        ...
        public void run() {
            OnlineSession session = SessionCache.fetch(player);
            ItemsDataStore items = ((ItemsDataStore) session.getDataStore(PrimaryType.Items));
            if(items != null) items.itemEnchant(location, itemStack);
        }
    }
    ...
    public static class ItemRepair implements Runnable {
        ...
        public void run() {
            ...
            OnlineSession session = SessionCache.fetch(player);
            ItemsDataStore itemsStore = ((ItemsDataStore) session.getDataStore(PrimaryType.Items));
            if(itemsStore != null) itemsStore.itemRepair(player.getLocation(), resultSlot);
        }
    }
