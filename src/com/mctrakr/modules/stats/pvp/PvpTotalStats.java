...
package com.mctrakr.modules.stats.pvp;
...
public class PvpTotalStats extends NormalData {
     ...
     */
    public PvpTotalStats(OnlineSession session, int victimId, ItemStack weapon) {
        super(session);
    ...
    public void fetchData() {
        QueryResult result = Query.table(PvpTotalsTable.TableName)
                ...
                .condition(PvpTotalsTable.PlayerId, session.getId())
        ...
        if(result == null) {
            Query.table(PvpTotalsTable.TableName)
                .value(PvpTotalsTable.PlayerId, session.getId())
