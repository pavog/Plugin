...
CREATE  TABLE IF NOT EXISTS `$dbname`.`$prefix_detailed_pve_kills` (
  ...
  PRIMARY KEY (`detailed_pve_id`, `material_id`, `creature_type`, `player_id`) ,
  INDEX `fk_player_id16_idx` (`player_id` ASC) ,
  INDEX `fk_material_id10_idx` (`material_id` ASC) ,
  INDEX `fk_creature_type2_idx` (`creature_type` ASC) ,
  ...
  CONSTRAINT `fk_material_id10`
    ...
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_creature_type2`
    FOREIGN KEY (`creature_type` )
    REFERENCES `$dbname`.`$prefix_creatures` (`creature_type` )
    ON DELETE CASCADE
    ON UPDATE NO ACTION);
...
CREATE  TABLE IF NOT EXISTS `$dbname`.`$prefix_total_pve_kills` (
  ...
  `creature_type` VARCHAR(45) NOT NULL ,
  `player_id` INT NOT NULL ,
  `player_killed` INT(10) NULL ,
  `creature_killd` INT(10) NULL ,
  PRIMARY KEY (`total_pve_id`, `creature_type`, `player_id`) ,
  INDEX `fk_player_id13_idx` (`player_id` ASC) ,
  INDEX `fk_creature_type1_idx` (`creature_type` ASC) ,
  CONSTRAINT `fk_player_id13`
    FOREIGN KEY (`player_id` )
    REFERENCES `$dbname`.`$prefix_players` (`player_id` )
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_creature_type1`
    FOREIGN KEY (`creature_type` )
    REFERENCES `$dbname`.`$prefix_creatures` (`creature_type` )
    ON DELETE CASCADE
    ON UPDATE NO ACTION);
